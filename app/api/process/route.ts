...
async function download(url: string): Promise<Buffer> {
  ...
  return Buffer.from(ab); // Fix für Blob/Buffer
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2) Replicate: Hintergrund entfernen
    ...
    let bytes = await download(fileUrl);

    // 4) Sharp-Optimierung (z. B. Hintergrund weiß oder transparent)
    let img = sharp(bytes).resize(1024, 1024, { fit: "inside" });

    if (bgMode === "white") {
      img = img.flatten({ background: { r: 255, g: 255, b: 255 } });
    } else if (bgMode === "transparent") {
      // nichts tun: Hintergrund bleibt transparent
    }

    // @ts-expect-error premultiply fehlt in den Types, geht aber runtime
    img = img.premultiply();
