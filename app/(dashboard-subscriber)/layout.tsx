// app/(dashboard-subscriber)/layout.tsx
import { ReactNode } from "react"
import { redirect } from "next/navigation"
import { createClient } from "@/lib/supabase/server"
import { AppSidebarSubscriber } from "@/components/(dashboard)/app-sidebar-subscriber"
import {
  SidebarProvider,
  SidebarInset,
  SidebarTrigger,
...
export default async function SubscriberDashboardLayout({ children }: { children: ReactNode }) {
  ...
  // Nur Subscriber haben Zugriff auf diesen Bereich
  ...
  return (
    <SidebarProvider>
      <AppSidebarSubscriber user={uiUser} />
      <SidebarInset className="flex min-h-dvh flex-col">
        ...
        <header className="sticky top-0 z-40 flex h-12 items-center gap-2 border-b bg-background px-3 sm:px-4">
          ...
          <div className="text-sm font-medium">Subscriber Dashboard</div>
        </header>
